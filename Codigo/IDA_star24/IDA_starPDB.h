...
public:
    ...
    **/
    signed char bounded_DFS(Node<T> n, signed char t, signed char g, PDB5& generator){
      ...
      for(int i=0;i<succs.size();++i){
        Node<T> m(&n, succs[i], generator.heuristic(succs[i].boardState));
        ++nodos_generados;
    ...
    **/
    signed char search(T sta, PDB5& generator){

      int startTime = getTimestamp();

      //Nodo inicial y su heuristica
      Node<T> result(sta,generator.heuristic(sta.boardState));
      ++nodos_generados;
